...
int VisionAllySensorRight::location(Enemy &enemy2, std::vector<std::shared_ptr<Enemy>> enemy, std::vector<int> &enemyIdOnTheField)
{
	...
	for (int i = 0; i < SIMULTANEOUS_NUMBER_OF_ENEMY_ON_THE_FIELD; ++i) {
		if (enemy2.getId() != enemy[enemyIdOnTheField[i]]->getId()){
			...
			kY = enemy[enemyIdOnTheField[i]]->position().y + Enemy::HEIGHT / 2;
